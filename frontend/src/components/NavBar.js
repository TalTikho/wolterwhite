...
export const Navbar = () => {
    ...
    const { search, setSearch, filters, updateFilter, clearFilters, cardsVisible, toggleCardsVisible } = useRestaurantFilter();
    ...
    useEffect(() => {
        ...
        setParams();
        return () => { if (profilePic !== "/knock.png") URL.revokeObjectURL(profilePic); };
    }, [token]);
    ...
    return (
        <nav className="navbar-container">
            ...
            <div className="navbar-search-wrapper">
                <input type="text" placeholder="🔍 Search restaurants..." value={search} onChange={e => setSearch(e.target.value)} className="navbar-search-input" />
                <div className="navbar-filter" ref={filterRef}>
                    <button type="button" onClick={() => setFilterOpen(prev => !prev)} className="navbar-btn navbar-btn-outline">Filter</button>
                    {filterOpen && (
                        <div className="navbar-filter-popup">
                            <label htmlFor="filter-address">Address contains</label>
                            <input id="filter-address" type="text" value={filters.address} onChange={e => updateFilter('address', e.target.value)} className="navbar-filter-input" />
                            <button type="button" onClick={clearFilters} className="navbar-btn navbar-btn-ghost">Clear</button>
                        </div>
                    )}
                </div>
                <button type="button" onClick={toggleCardsVisible} className="navbar-btn navbar-btn-outline">{cardsVisible ? 'Clear' : 'Show'}</button>
            </div>
            <div className="navbar-controls">
                ...
                <div className="navbar-profile-section">
                    {token ? (
                        <>
                            {user?.role === 'admin' && <Link to="/admin" className="navbar-btn navbar-btn-outline">Admin</Link>}
